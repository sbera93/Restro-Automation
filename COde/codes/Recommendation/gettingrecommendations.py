# -*- coding: utf-8 -*-
"""
Created on Thu Feb  6 19:56:25 2020
...
def predictRating(recommend_data,item1,user1):
    ...
    toBeRated["item"].append(item1)
    toBeRated["user"].append(user1)
...
def dataofuseroutput(user,item):
    recommend_data = readingFile("test.csv")
    op=predictRating(recommend_data,item,user)
    print("user "+str(user)+" will give rating "+str(op)+" to item "+str(item))
    return op
    
dataofuseroutput(3,1)